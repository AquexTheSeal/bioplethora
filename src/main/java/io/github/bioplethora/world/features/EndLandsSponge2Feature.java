...
public class EndLandsSponge2Feature extends Feature<NoFeatureConfig> {

    public boolean isHighlands;

    public EndLandsSponge2Feature(Codec<NoFeatureConfig> pCodec, boolean isHighlands) {
        super(pCodec);
    ...
    public boolean place(ISeedReader world, ChunkGenerator chunkGen, Random rand, BlockPos pos, NoFeatureConfig config) {
        ...
        if (isHighlands) {
            this.placeSponge(0.5D + rand.nextDouble(), 12, world, rand, pos);
        } else {
            this.placeSponge(0.5D, 7, world, rand, pos);
        }
        return true;
    }

    public void placeSponge(double radIncrement, int randomYHeight, ISeedReader world, Random rand, BlockPos pos) {
        int radius = 0;
        if (checkPlacement(world, pos)) {
            int yRand = 8 + rand.nextInt(randomYHeight);
            double radHelper = radius;
            for (int sy = -yRand; sy <= 0; sy++) {
                radHelper += radIncrement;
