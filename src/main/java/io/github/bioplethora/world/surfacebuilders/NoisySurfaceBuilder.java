...
public class NoisySurfaceBuilder extends SurfaceBuilder<NoisySurfaceBuilderConfig> {
    ...
    public NoisySurfaceBuilder(Codec<NoisySurfaceBuilderConfig> codec) {
        super(codec);
    }

    public void apply(Random random, IChunk chunk, Biome biome, int x, int z, int height, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, NoisySurfaceBuilderConfig config) {
        this.apply(random, chunk, biome, x, z, height, noise, defaultBlock, defaultFluid, config.getTopMaterial(), config.getTopMaterialUncommon(), config.getUnderMaterial(), config.getUnderwaterMaterial(), seaLevel);
    }

    protected void apply(Random random, IChunk chunk, Biome biome, int x, int z, int height, double noise, BlockState defaultBlock, BlockState defaultFluid, BlockState topMaterial, BlockState uncommonTopMaterial, BlockState underMaterial, BlockState underwaterMaterial, int seaLevel) {
                ...
                if (i == -1) {
                    ...
                    if (i1 < seaLevel && (state == null || state1 == null || state.isAir())) {
                        if (biome.getTemperature(blockpos$mutable.set(x, i1, z)) < 0.15F) {
                            ...
                            state1 = Blocks.ICE.defaultBlockState();
                        } else {
                            state = defaultFluid;
                            state1 = defaultFluid;
                        }
